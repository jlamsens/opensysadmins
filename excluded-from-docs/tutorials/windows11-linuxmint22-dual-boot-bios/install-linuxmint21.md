...
=== "Step1"
    [In UEFI-mode](../../../docs/tutorials/OLD/windows11-linuxmint21-dual-boot-uefi/something-else.png), the Linux Mint 21.x installer finds the "Windows 11 installation", but for one reason or another (bug?) not in BIOS (non-CSM) mode (in this case). Nevermind. Just select "Something else" (after installation, GRUB will find the "Windows 11 installation" anyway).
